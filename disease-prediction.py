...
df.replace({'prognosis':{'Fungal infection':0,'Allergy':1,'GERD':2,'Chronic cholestasis':3,'Drug Reaction':4,
    ...
    'Impetigo':40,'Corona virus':41}},inplace=True)
...
def SVM():
    if len(NameEn.get()) == 0:
        ...
        conn = sqlite3.connect('database.db') 
        c = conn.cursor() 
        c.execute("CREATE TABLE IF NOT EXISTS SVM(Name StringVar,Symtom1 StringVar,Symtom2 StringVar,Symtom3 StringVar,Symtom4 StringVar,Symtom5 StringVar,Disease StringVar)")
        c.execute("INSERT INTO SVM(Name,Symtom1,Symtom2,Symtom3,Symtom4,Symtom5,Disease) VALUES(?,?,?,?,?,?,?)",(NameEn.get(),Symptom1.get(),Symptom2.get(),Symptom3.get(),Symptom4.get(),Symptom5.get(),pred5.get()))
        conn.commit()  
        c.close() 
        conn.close()
        #printing scatter plot of disease predicted vs its symptoms
...
lrLb = Label(root, text="DecisionTree", fg="yellow", bg="red", width = 20)
...
ranfLb = Label(root, text="NaiveBayes", fg="green", bg="light green", width = 20)
...
svmLb = Label(root, text="SVM", fg="purple", bg="pink", width = 20)
svmLb.config(font=("Times",15,"bold italic"))
svmLb.grid(row=23, column=0, pady=10, sticky=W)
OPTIONS = sorted(l1)
...
lr = Button(root, text="Prediction 3", command=NaiveBayes,bg="light green",fg="green")
...
sv = Button(root, text="Prediction 5", command=SVM,bg="pink",fg="purple")
...
t1=Label(root,font=("Times",15,"bold italic"),text="Decision Tree",height=1,bg="Red"
         ,width=40,fg="yellow",textvariable=pred1,relief="sunken").grid(row=15, column=1, padx=10)
...
t3=Label(root,font=("Times",15,"bold italic"),text="Naive Bayes",height=1,bg="light green"
         ,width=40,fg="green",textvariable=pred3,relief="sunken").grid(row=19, column=1, padx=10)
...
t5=Label(root,font=("Times",15,"bold italic"),text="SVM",height=1,bg="pink"
         ,width=40,fg="purple",textvariable=pred5,relief="sunken").grid(row=23, column=1, padx=10)
